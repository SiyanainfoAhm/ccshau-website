#!/usr/bin/env node
/**
 * Fill AAREM International MoU + National MoU sidebar content from legacy:
 *   https://hau.ac.in/page/international-mo-u
 *   https://hau.ac.in/page/national-mo-u
 * (linked from https://hau.ac.in/department/MjA=/NzI=)
 *
 * Uploads MoU PDFs to Azure and rewrites hrefs.
 *
 *   node scripts/ops/sync-aarem-mou-sidebars.mjs
 *   node scripts/ops/sync-aarem-mou-sidebars.mjs --apply
 */
...
const CACHE = join(__dirname, "../legacy-import/reports/aarem-mou-cache");
const CONTAINER =
...
const TABS = [
  {
    labelEn: "International MoU",
    labelHi: "अंतरराष्ट्रीय समझौता ज्ञापन",
    legacyUrl: "https://hau.ac.in/page/international-mo-u",
  },
  {
    labelEn: "National MoU",
    labelHi: "राष्ट्रीय समझौता ज्ञापन",
    legacyUrl: "https://hau.ac.in/page/national-mo-u",
  },
];
...
function extractHauFileUrls(html) {
  const urls = new Set();
  for (const m of html.matchAll(/https?:\/\/(?:www\.)?hau\.ac\.in\/(?:storage\/app\/uploads|public\/)[^"'\\\s>]+/gi)) {
...
function toHindiShell(html) {
  // Keep same file links; translate common table headers / titles only
  return html
    .replace(/MoUs for International Collaboration/gi, "अंतरराष्ट्रीय सहयोग हेतु समझौता ज्ञापन")
    .replace(/MoUs for National Collaboration/gi, "राष्ट्रीय सहयोग हेतु समझौता ज्ञापन")
    .replace(/Sr\.\s*No\./gi, "क्रमांक")
    .replace(/Name of Organization \(Purpose\)/gi, "संगठन का नाम (उद्देश्य)")
    .replace(/Date of signing/gi, "हस्ताक्षर तिथि")
...
async function main() {
  ...
  for (const tab of TABS) {
    ...
    if (!content || content.length < 50) throw new Error(`No content for ${tab.labelEn}`);
  ...
  if (!APPLY) {
    for (const p of prepared) {
      console.log(`\n${p.labelEn} preview:`, p.content.replace(/\s+/g, " ").slice(0, 220));
    }
    console.log("\nPass --apply to upload PDFs + update sidebar content.");
    return;
  }
  ...
  for (const f of allFiles) {
    try {
      ...
      urlMap.set(f, f); // keep legacy if upload fails
    }
  }
