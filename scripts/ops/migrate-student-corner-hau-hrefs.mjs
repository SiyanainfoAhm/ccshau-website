#!/usr/bin/env node
/**
 * Migrate ccshau_page_student_corner_items.href (hau.ac.in files) → Azure.
 * Also migrates file_path when it is a HAU URL.
 *
 *   node scripts/ops/migrate-student-corner-hau-hrefs.mjs
 *   node scripts/ops/migrate-student-corner-hau-hrefs.mjs --apply
 */
...
const CACHE = join(__dirname, "../legacy-import/reports/student-corner-file-cache");
const CONTAINER =
  process.env.NEXT_PUBLIC_AZURE_STORAGE_CONTAINER?.trim() ||
  process.env.AZURE_STORAGE_CONTAINER?.trim() ||
  "ccshaucontainer";
const TABLE = "ccshau_page_student_corner_items";
...
/** Only migrate downloadable files; skip HTML/PHP portal links. */
function isMigratableFileUrl(u) {
  if (!isHauUrl(u)) return false;
  try {
    const path = new URL(u.trim()).pathname.toLowerCase();
    if (/\.(pdf|jpe?g|png|gif|webp|docx?|xlsx?|pptx?|zip|rar|txt|csv)$/i.test(path)) return true;
    if (/\/(public\/notification-documents|storage\/app\/uploads|storage\/app\/public)\//i.test(path)) {
      return true;
    }
    return false;
  } catch {
...
async function fetchAll(supabase) {
  ...
  return rows.filter(
    (r) => isMigratableFileUrl(r.href) || isMigratableFileUrl(r.file_path),
  );
}
...
async function main() {
  ...
  for (const r of rows) {
    if (isMigratableFileUrl(r.href)) uniqueUrls.add(r.href.trim());
    if (isMigratableFileUrl(r.file_path)) uniqueUrls.add(r.file_path.trim());
  }

  // Non-file HAU links (e.g. GreenForms PHP) — report only, leave as-is
  const { data: allHau } = await supabase
    .from(TABLE)
    .select("id, title_en, href, file_path")
    .or("href.ilike.%hau.ac.in%,file_path.ilike.%hau.ac.in%");
  const skipped = (allHau ?? []).filter(
    (r) =>
      (isHauUrl(r.href) && !isMigratableFileUrl(r.href)) ||
      (isHauUrl(r.file_path) && !isMigratableFileUrl(r.file_path)),
  );

  console.log(`student corner rows with HAU file urls: ${rows.length}`);
  console.log(`unique HAU files: ${uniqueUrls.size}`);
  console.log(`non-file HAU links skipped: ${skipped.length}`);
  for (const s of skipped) console.log(`  SKIP ${s.title_en}: ${s.href ?? s.file_path}`);
  ...
  await mapPool(rows, 3, async (row, idx) => {
    try {
      ...
      for (const field of ["href", "file_path"]) {
        const val = row[field];
        if (!isMigratableFileUrl(val)) continue;
  ...
  console.log(`  remaining HAU student corner urls: ${remaining.length}`);
