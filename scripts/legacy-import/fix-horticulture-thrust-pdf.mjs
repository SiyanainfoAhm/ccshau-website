/**
 * Fix Horticulture (hisar-horticulture) Thurst Area tab to use legacy PDF.
 * Legacy menu detail has page_id=null but link=page/thurst-area-2 → cms file 1687836669.pdf
 *
 * Usage:
 *   node fix-horticulture-thrust-pdf.mjs --dry-run
 *   node fix-horticulture-thrust-pdf.mjs --confirm
 */
...
const PAGE_SLUG = "hisar-horticulture";
const SIDEBAR_ID = "783d545a-4c13-4630-a9b8-a96799ed7321";
const FILE_NAME = "1687836669.pdf";
...
async function main() {
  ...
  const content = pdfHtml(azureUrl, "Thrust Area");
  console.log(`page=${PAGE_SLUG} sidebar=${SIDEBAR_ID}`);
  ...
  if (CONFIRM) {
    const { error } = await supabase
      ...
      .update({
        label_en: "Thrust Area",
        content_en: content,
        href: null,
      })
      .eq("id", SIDEBAR_ID);
    if (error) throw new Error(error.message);
    console.log("updated sidebar Thurst Area → Thrust Area + PDF");
  } else {
